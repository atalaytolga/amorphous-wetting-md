...
function main(args)
    local box_length = {12, 12, 100}
    local dimension = #box_length
    local fluid_density = 0.70
    ...
    local slab = {1, 1, 1/4}
    local nparticle = math.floor(fluid_density * numeric.prod(slab) * numeric.prod(box_length))


    local particle = mdsim.particle({dimension = dimension, particles = nparticle})

    -- set initial particle positions
    mdsim.positions.lattice({box = simulation_box, particle = particle, slab = slab}):set()
    ...
    definitions.lennard_jones.create_pair_force({
        box = simulation_box, particle = particle, cutoff = args.cutoff, smoothing = args.smoothing
    })
    ...
    -- run simulation
    observables.sampler:run(steps / 4)
    ...
    observables.sampler:run(steps - steps / 4)
