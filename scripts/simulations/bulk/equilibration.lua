#!/usr/bin/env halmd
-- Equilibrate a bulk Lennard-Jones system.
...
function main(args)
    ...
    local unit_edge = (volume / numeric.prod(ratios)) ^ (1 / dimension)
    local length = {}
    for i = 1, #ratios do
    ...
    local sphere = mdsim.geometries.sphere({
        centre = numeric.scalar_vector(dimension, 0),
        radius = numeric.min(length) / 4
    })
    local sphere_group = mdsim.particle_groups.region({
        particle = particle,
        geometry = sphere,
        selection = "included",
    ...
    if interval > 0 then
        msv = observables.thermodynamics({
            box = box,
            group = all_group
        })
        msv:writer({
...
function define_args(parser)
    ...
    parser:add_argument("ratios", {
        ...
        default = {1, 1, 1},
        help = "relative aspect ratios of simulation box"
    })
